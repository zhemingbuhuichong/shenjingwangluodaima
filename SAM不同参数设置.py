...
rhos = [0.005,0.01, 0.1, 0.2]  # SAM的扰动范围
...
def main():
    for rho in rhos:
        model = init_model()
        optimizer = SAM(model.parameters(), base_optimizer=optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=weight_decay), rho=rho)
        ...
        config_name = f'SAM_rho{rho}_weight_decay{weight_decay}'
