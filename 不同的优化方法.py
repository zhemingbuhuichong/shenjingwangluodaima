...
# 初始化模型
def init_model():
    model = ResNet18(BasicBlock, [2, 2, 2, 2], num_classes=100)  # CIFAR-100 有 100 个类别
...
class SAM(optim.Optimizer):
    def __init__(self, params, base_optimizer, rho=0.05, efficient=False, k_sam=False, k_steps=3, **kwargs):
        defaults = dict(rho=rho, efficient=efficient, k_sam=k_sam, k_steps=k_steps, **kwargs)
    ...
    def first_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                e_w = p.grad * scale
                if group['k_sam']:
                    for _ in range(group['k_steps']):
                        p.add_(e_w)  # K-step perturbation
                else:
...
def save_results(config_name, train_losses, train_accuracies, test_losses, test_accuracies, train_time, final_train_acc, final_test_acc):
    output_dir = os.path.join('output3', config_name)
...
def main():
    algorithms = [
        {'name': 'SAM', 'efficient': False, 'k_sam': False},
        {'name': 'Efficient_SAM', 'efficient': True, 'k_sam': False},
        {'name': 'K_SAM', 'efficient': False, 'k_sam': True, 'k_steps': 3}
    ]

    for algo in algorithms:
        model = init_model()
        optimizer = SAM(model.parameters(), base_optimizer=optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=weight_decay),
                        rho=rho, efficient=algo['efficient'], k_sam=algo['k_sam'], k_steps=algo.get('k_steps', 1))
        ...
        for epoch in range(1, epochs + 1):
            train_loss, train_acc = train(epoch, model, optimizer, scheduler)
        ...
        config_name = f'{algo["name"]}_rho{rho}_weight_decay{weight_decay}'
